...
class ViewController: UIViewController {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vc = storyboard?.instantiateViewController(identifier: "SecondaryViewController") as? SecondaryViewController {
            vc.stateImage = UIImage(named: stateNames[indexPath.row])!
            vc.stateName = stateNames[indexPath.row]
